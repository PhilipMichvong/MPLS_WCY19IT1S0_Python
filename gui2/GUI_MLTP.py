...
# zmienne ip dla kolejnych routerow

global ipr1, ipr2, ipr3, ipr4

# maska dla kolejych routerow

global maskr1, maskr2, maskr3, maskr4

# interfac'y dla kolejnych routerow

global intr1, intr2, intr3, intr4

# ustawuenua do adresacji r1
...
def Router_Add():
    ...
    label2_router.bind("<Button-1>", open_setings_r2)
    label2_router.place(x=760, y=80)
    label3_router = Label(image=img3)
    label3_router.bind("<Button-1>", open_setings_r3)
    label3_router.place(x=200, y=300)
    label4_router = Label(image=img4)
    label4_router.bind("<Button-1>", open_setings_r4)
...
def ToolBox():
    consoleLabel = tk.Label(root, text="Konsola symulacji",
    ...
    ComputerAddLabel = tk.Label(
        root, text="Dodaj komputer", font=('Arial', 15))
    RouterAddButton = tk.Button(root, text="router", font=(
        'arial', 10), command=lambda: Router_Add())
    ComputerAddButton = tk.Button(root, text="komputer", font=(
        'arial', 10), command=lambda: Computer_Add())
...
def ConsoleOutputPart():
    ...
    consoleOutputLabel = tk.Label(
        root, text=Console_Text, font=('Arial', 8), fg="#FFF", bg="#000")
